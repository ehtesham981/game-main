              ...
              <div className="bg-white rounded-[4rem] shadow-[0_40px_100px_-20px_rgba(0,0,0,0.05)] border border-slate-100 p-10 md:p-16">
                <form onSubmit={handleInitialSubmit} className="space-y-12">
                  ...
                  <button
                    ...
                    className={`w-full py-8 text-white font-black rounded-[2.5rem] transition-all shadow-2xl flex items-center justify-center gap-6 uppercase tracking-[0.4em] text-xs transform active:scale-[0.98] ${isBalanceEnough ? 'bg-slate-900 hover:bg-indigo-600 shadow-indigo-100' : 'bg-slate-200 text-slate-400 cursor-not-allowed'
                      }`}
                  >
              ...
              recentUserTasks.map(t => (
                <div key={t.id} className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all group">
                  <div className="flex justify-between items-start mb-6">
                    ...
                    <div className={`px-3 py-1 text-[8px] font-black rounded-lg uppercase tracking-widest border transition-all ${t.status === 'active' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-amber-50 text-amber-600 border-amber-100 animate-pulse'
                      }`}>
                      {t.status}
                    </div>
                  </div>
